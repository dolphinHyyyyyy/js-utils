import { binaryToString, md5Binary, md5Core, stringToBinary } from "./md5"
...
export function binaryToBase64(value: number[], padding = "") {
	...
	for (let i = 0; i < value.length * 4; i += 3) {
		...
		for (let j = 0; j < 4; j++) {
			result += i * 8 + j * 6 > value.length * 32 ? padding : ("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".charAt((triplet >> 6 * (3 - j)) & 0x3F))
		}
	}
	return result
}
