 ...
 * @param callback.days 需要显示的剩余天数
 * @param callback.hours 需要显示的剩余小时数
 * @param callback.minutes 需要显示的剩余分数
 * @param callback.seconds 需要显示的剩余秒数
 * @param callback.total 剩余总秒数
 * @param now 当前时间。可以由服务器下发以避免用户设置时间的误差
 * @returns 返回一个计时器，可以通过 `clearInterval()` 停止倒计时
 * @example countDown(new Date("2020/1/1"), (day, hour, minute, second) => console.log(day, hour, minute, second))
 */
export function countDown(end: Date, callback: (days: string, hours: string, minutes: string, seconds: string, total: number) => void | boolean, now = new Date()) {
 ...
 * @param end 开始计时的时间
 * @param callback 每隔一秒执行的回调函数，可以用于更新界面。函数的参数依次为:
 * @param callback.years 需要显示的总年数
 * @param callback.days 需要显示的总天数
 * @param callback.hours 需要显示的总小时数
 * @param callback.minutes 需要显示的总分数
 * @param callback.seconds 需要显示的总秒数
 * @param callback.total 已经过的总秒数
 * @param now 当前时间。可以由服务器下发以避免用户设置时间的误差
 * @returns 返回一个计时器，可以通过 `clearInterval()` 停止倒计时
